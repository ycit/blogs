...
> 倒排索引由文档中所有不重复词的列表构成，对于每个词，有一个包含它的文档列表。
>
...
> > *近实时搜索*
> >
> > 上述的按段搜索，一个新的文档从索引到可被搜索的延迟显著降低。但是还不够快，磁盘称为了瓶颈，提交一个段到磁盘需要一个 fsync 来确保段被物理性地写入磁盘，这样再断电的时候就不会丢失数据。但 fsync 代价比较大，如果每次索引一个文档都需要执行 fsync，会造成性能问题。
> >
...
>   > 客户端向协调节点 发送请求
>   >
>   > 协调节点使用文档 _id 确定文档属于分片0，分片0 的存在于所有三个节点上，根据负载均衡，将请求转发到 node2;
>   >
>   > node2 将文档返回给 协调节点node1, 协调节点node1 返回给客户端；
>   >
...
>    >
> > 每个分片返回各自优先队列中所有文档的id 和 排序值给协调节点，即node3,协调节点 node3 合并这些值到自己的优先队列中来产生一个全局排序后的结果列表
>
>   - 取回阶段
>
